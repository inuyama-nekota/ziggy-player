...
export function parseLrc(source) {
  if (typeof source !== "string") throw new TypeError("LRC data must be text.");
  ...
  for (const rawLine of lines) {
    ...
    if (metadataMatch && !/^\d+$/.test(metadataMatch[1])) {
      ...
      metadata[key] = value;
      if (key === "offset" && Number.isFinite(Number(value))) offset = Number(value);
      continue;
    }
    ...
    for (const match of timestamps) {
      ...
      entries.push({
        timeMs: minutes * 60_000 + seconds * 1_000 + fractionToMilliseconds(match[3]),
        text,
      });
    }
  }
  ...
  if (adjusted.length === 0) throw new Error("時刻付きの歌詞行が見つかりませんでした。");
